 ...
 */
const log = (url: string, status: number, body?: any) => {
...
const post = async (url: string, body: any, opt?: OptionsOpt) => {
    const { verbose } = getOptions(opt);
...
class RollupClient {
    ...
    constructor({ signer, tezosUrl, rollupUrl, verbose }: { signer: Signer, tezosUrl: string, rollupUrl: string, verbose?: boolean }) {
    ...
    async send(payload: string): Promise<string> {
        ...
        const previousCounter = await get(`${tezosUrl}/chains/main/blocks/head/context/contracts/tz1QFD9WqLWZmmAuqnnTPPUjfauitYEWdshv/counter`, options);
        const counter = Number.parseInt(previousCounter) + 1; // We direct increment the counter

        // The hash of head~2
        const previousHashForSimulation = await get(`${tezosUrl}/chains/main/blocks/head~2/hash`, options);
        ...
        const simulatedOperation = await post(`${tezosUrl}/chains/main/blocks/head/helpers/scripts/simulate_operation`, operationToSimulate, options);
        // Retrieve the needed gas 
        const estimatedGas = simulatedOperation.contents[0].metadata["operation_result"]["consumed_milligas"];

        // The hash of head~2
        const previousHash = await get(`${tezosUrl}/chains/main/blocks/head~2/hash`, options);
        ...
        const forgedOperationResult = await post(`${tezosUrl}/chains/main/blocks/head/helpers/forge/operations`, operationToForge, options);
        ...
        const opHash = await post(`${tezosUrl}/injection/operation?chain=main`, toInject, options);
    ...
    async getState(path: string) {
        ...
        const res = await get(`${rollupUrl}/global/block/head/durable/wasm_2_0_0/value?key=${path}`, options);
        return res;
    }
    ...
    async getSubkeys(path: string) {
        const options = { verbose: this.verbose };
        const rollupUrl = this.rollupUrl;
        const res = await get(`${rollupUrl}/global/block/head/durable/wasm_2_0_0/subkeys?key=${path}`, options);
        return res;
    }
}
